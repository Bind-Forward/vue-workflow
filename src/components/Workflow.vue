<template>
    <div class="flow-layout">
        <div style="top: 10px;left: 30px;position: absolute;z-index: 66;">
            <button @click="saveListData" style="padding:5px 8px;cursor:pointer">保存</button>
        </div>
        <div class="flow-editor">
            ...
            <div class="canvas-container" :data-zoom="canvasDataRoom">
                <div id="campaignCanvas" :style="canvasRoomScaleStyle" ref="campaignCanvas">
                    <template v-for="(flowItem,index) in flowList">
                        <div class="node-content-wrap" :key="index" @mousedown="handleFlowMoveDown"
                            @mouseup="(event)=>{handleChangeFlowPosition(flowItem,event)}" :id="flowItem.uuid"
                            @click.stop="(event)=>{handleEditFlowItem(flowItem,event)}"
                            :style="{left:flowItem.left+'px',top:flowItem.top+'px'}"
                            @contextmenu.prevent="(event)=>{dealdbclick(flowItem,event)}">
                            <div class="node-content">
                                ...
                                <template v-if="flowItem.groupType === flowTypeConstant.temp ">
                                    <draggable class="flow-item node-temp node-temp-img" ref="tempNode"
                                        :id="flowItem.uuid" :group="{name:'sortable', pull:false, put: true }">
                                    </draggable>
                                </template>
                            </div>
                        </div>
            ...
            <div class="flow-editor-sidebar">
                <template v-for="(flowItem,index) in flowAllList">
                    <draggable class="items-box" :key="index" :list="flowItem.children"
                        :group="{name:'sortable', pull: 'clone', put: false }" v-bind="dragConfig"
                        :move="handleFlowMoveItem" @start="handleFlowMoveStart" @end="handleFlowMoveEnd" :sort="false"
                        :ref="flowItem.ref">
                        <template v-for="(item,index) in flowItem.children">
                            <template v-if="flowItem.type === flowTypeConstant.action">
                                <div class="node" :key="index" :title="item.name" :data-id="item.id"
                                    :data-type="item.type" :data-group="item.groupType" v-if="!item.hidden">
        ...
        <el-dialog class="dialog-wrap" :visible.sync="dialogObj.visible" :show-close=false width="500px">
...
<script>
    ...
    import FlowDataList from "../constant/dataList";
    import {
        clone,
        uuid
    } from "../utils";
    ...
    export default {
        ...
        methods: {
            ...
            drawLines(flowItem) {
                // isStartFlowItem isEndFlowItem isTempFlowItem isNodeFlowItem isIfFlowItem isExpandFlowItem
                if (this.isStartFlowItem(flowItem)) {
                    this.draggableFlowConnect(flowItem.uuid, flowItem.next[0]);
                } else if (this.isTempFlowItem(flowItem)) {
                    this.draggableFlowConnect(flowItem.prev[0], flowItem.uuid);
                } else if (this.isNodeFlowItem(flowItem)) {
                    this.draggableFlowConnect(flowItem.uuid, flowItem.next[0]);
                } else if (this.isIfFlowItem(flowItem.type)) {
                    this.draggableFlowConnect(flowItem.uuid, flowItem.nextIfId);
                    this.createFlowItemLabel(flowItem.uuid, flowItem.nextIfId, '是');
                    this.draggableFlowConnect(flowItem.uuid, flowItem.nextElseId);
                    this.createFlowItemLabel(flowItem.uuid, flowItem.nextElseId, '否');
                } else if (this.isExpandFlowItem(flowItem.type)) {
                    flowItem.formData.ruleGroupList.forEach(item => {
                        this.draggableFlowConnect(flowItem.uuid, item.nextStep);
                        this.createFlowItemLabel(flowItem.uuid, item.nextStep, item.name);
                    })
                } else {
            ...
            createFlowItemLabel(source, target, label) {
                this.$nextTick(() => {
                    ...
                    if (lines.length > 0) {
                        lines[0].getOverlay("flowItemDesc").setLabel(
                    ...
                    if ((flowItem.type === FLOW_ITEM_TYPE.endNode)) {
                        if (preFlowItem) {
                            ...
                            if (this.isIfFlowItem(preFlowItem.type)) {
                                ...
                                // create one flow item
                                let tempFlowUuid = this.addExpandOneTempFlowItem(preFlowItem, flowItem.left,
                                    expandFlowItem.name);
                    ...
                    if (this.isExpandFlowItem(flowItemType)) {
                        ...
                        if (addRuleGroupList.length > 0) {
                            ...
                            addRuleGroupList.forEach((ruleGroupItem, index) => {
                                ...
                                let tempFlowUuid = this.createFlowItem(FLOW_ITEM_TYPE.tempNode, newFlowItem
                                    .uuid, {
                                        left
                                    });
            ...
            //todo1
            addDataTempFlowItem(preUuid, nextUuid) {
                this.draggableFlowConnect(preUuid, nextUuid);
            },
            //
            addIfTempFlowItem(preUuid) {
                ...
                // if
                let tempFlowUuid = this.createFlowItem(FLOW_ITEM_TYPE.tempNode, preUuid, {
                    offsetLeft: -FLOW_LEFT_STEP_LENGTH
                });
                this.draggableFlowConnect(preUuid, tempFlowUuid);
                this.createFlowItemLabel(preUuid, tempFlowUuid, '是');
                preTempItem.nextIfId = tempFlowUuid;
                //  else
                let tempFlowUuid2 = this.createFlowItem(FLOW_ITEM_TYPE.tempNode, preUuid, {
                    offsetLeft: FLOW_LEFT_STEP_LENGTH
                });
            ...
            addExpandOneTempFlowItem(preFlowItem, left, expandFlowItemName) {
                let tempFlowUuid = this.createFlowItem(FLOW_ITEM_TYPE.tempNode, preFlowItem.uuid, {
                    left: left
                });
            ...
            isNodeFlowItem(flowItem) {
                return flowItem.type === FLOW_ITEM_TYPE.nodeNode;
            },
